...
Здесь собрана информация о наиболее важных по моему мнению статьях за последние несколько лет о сегментации данных, 
полученных с лидара, и классификации точек. Для каждой статья выписана следующая информация: какая задача решается
...
а как применять свертки к неструктурированным данным (к примеру графам). Я собирала эту информацию для себя и верю, 
что она может быть кому-то полезна.

#### существующие датасеты

Для начала - с какими данными мы работает
* [китти](http://www.cvlibs.net/datasets/kitti/) - датасет для беспилотных автомобилей. Данные с лидаров и изображения 
дорожной обстановки. Для решения задач детекции, классификации участников дорожного движения, треккинга объектов.
...
* [ModelNet40](http://modelnet.cs.princeton.edu/) - то же самое, объекты разных форм
...
Основополашающими мне показались следующие архитектуры:
* PointNet
* PointNet++
* DGCNN

Большая часть исследователей используют их как базовую арзитектуру, улучшая и дополняя. Далее пойдет описание архитектур.
 *  [PointNet: Deep Learning on Point Sets for 3D Classification and Segmentation](https://arxiv.org/abs/1612.00593)
    * что решают:  Задачи классификации и сегментации в point clouds.
    * особенности: Первопроходцы в работе с неотсортированными точками. то есть результат не зависит от того, в каком 
    порядке подадутся на вход точки
    * как решают: Одна сеть с двумя головами. Модель состоит из сети для определения трансформации, собственно 
    применении подсчитанного преобразования к точкам. Далее идут преобразования, применяемые к каждой точки 
    (однослойная сеть). В конце мы с помощью maxpooling создаем глобальный вектор признаков лоя всей картинки.
     Ну и классифицируем полносвязной сетью, которая работает с поддсчитанным признаком и дает классы. 
     Или отдаем этот вектор на вход сети для сегментации.
...
* [PointNet++: Deep Hierarchical Feature Learning on Point Sets in a Metric Space](https://arxiv.org/abs/1706.02413)
  * что решают: Задачи классификации и сегментации в point clouds.
  * как решают: рекурсивно применяют pointNet к более мелким подоблакам, по пнплогии со сверточными сетями.
 ...
 * [Dynamic Graph CNN for Learning on Point Clouds](https://arxiv.org/abs/1801.07829)
   * что решают: Задачи классификации и сегментации в point clouds. 
   * как решают: переводят имеющиеся точки в граф: вершины - точки, ребра существуют только между k ближайшими. 
   Потом считают свертки на ближайших точках к текущей, делают пулинг и полученное значение записывают как текущий 
   эмбединг точки. Используют локальные и глобальные координаты как входные данные для свертки. Прекрасный обзор 
   других методов решения.
![архитектура DGCNN](https://habrastorage.org/webt/ov/zk/h7/ovzkh78zz0cnczzdp0notx8u6ea.png)
